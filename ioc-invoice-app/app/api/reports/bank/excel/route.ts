...
import { loadBankReportDataset, type BankReportPeriod } from "@/lib/reports/load-bank-report";
import { generateBankExcelReport } from "@/lib/reports/bank-excel-report";

export const maxDuration = 120;

function periodFromBody(body: Record<string, unknown>): BankReportPeriod | null {
...
export async function POST(request: NextRequest) {
  ...
  try {
    const data = await loadBankReportDataset(period);
    const { buffer, filename } = await generateBankExcelReport(data);
    return binaryFileResponse(buffer, filename, EXCEL_CONTENT_TYPE);
  } catch (err) {
    const message = err instanceof Error ? err.message : "Bank Excel report failed";
    console.error("[reports/bank/excel]", message);
